截至 2026-09-09 06:37，最新完成的标准 16 题评测得分为 10/16（62.5%）；新一轮完整 LoCoMo `conv-43` 已启动，正在从空库写入历史，尚无本轮问答成绩。前一轮 152 次历史回执全部校验通过，检索、回答和判分为实际执行，没有新的历史写入或写入模型调用。已完成的旧评测和失败记录均保留。
...
两个修补已无冲突合成后续候选 `8717d918`，703 项测试及构建通过，144 个产物两份哈希一致。上述 16 题仍归属 `9c7cf1b`；集成候选正在进行新的真实评测，尚无完成成绩，未部署到主服务。

新一轮 `priority-conv43-20260909-01` 于上海时间 06:36:44 启动，使用集成候选 `8717d918` 和新数据库，保留原圈定 LoCoMo `conv-43` 的完整 29 个会话、680 条消息、43 次原分块 add 和 47 道原题，其中 24 道时间题、23 道多跳题。判分服务按原协议完成一次标准预检后，评测器于 06:37:06 开始写入；controller PID 为 80249，进程独立持续运行。实际配置已确认 queryFocus=true、8000 毫秒上限、json_object、32 条证据、6000 token 预算、候选池 200。该运行用于扩大本轮覆盖范围，不再仅围绕这 16 题调参。

本轮完整保留 LoCoMo 原 `upstream-reproduction + refined-python` 协议：本地 `qwen3:14b` Q4_K_M Judge、GLM-5.2 Answer，单 worker；本地量化 Judge 不代表官方榜单模型等价。运行计划及入口已冻结，三项本地测试通过，详见 [conv-43 准备与启动报告](/Users/enoch/Workspace/comp/agent-memory-workspace/reports/priority-conv43-preparation-20260909.json)。耗时将按实际记录汇总，当前尚不能给出本轮分数。
