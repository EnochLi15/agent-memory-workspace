截至 2026-09-09 07:19，LoCoMo `conv-43` 的恢复运行已完成 43/43 次原历史请求，47 题问答正在进行，尚无最终成绩。它复用了先前 38 块真实历史，新写入最后 5 块；先前运行于 06:54:25 因第 39 块的状态比较容量错误结束，47 题均为 `service_error`，固定 0/47 的旧结果保留。此前 MemoryOps 的 16 题为 10/16（62.5%），分别记录。容量问题已完成确定性复现和分批修补，713 项测试通过。
...
本次第 39 块重新抽取后仅需 6 对状态比较、20,346 字符，一次标准模型调用后成功；另一次新写为 12 对、38,423 字符，也只需一批。本轮尾部尚未在真实模型上触发多批，不能将一次成功全部归因于分批修补。原 27 对超限输入的无损覆盖和提交契约已用精确录制输入及合成回归独立验证，真实问答质量仍以当前 47 题结果为准。
