...
const parentIndex=args.indexOf('--parent-run');if(parentIndex>=0&&!args[parentIndex+1])throw Error('Missing --parent-run path');
const prior=parentIndex>=0?args[parentIndex+1]:'eval/artifacts/holdout-v2-U3-memops';
...
if(args.includes('--erasure-binding'))stageEnv.MEMORY_ERASURE_BINDING='true';
if(args.includes('--semantic-transitions')){stageEnv.MEMORY_SEMANTIC_TRANSITIONS='true';stageEnv.MEMORY_SOURCE_ERASURE='true';stageEnv.MEMORY_ERASURE_BINDING='true';}
if(args.includes('--source-erasure')){stageEnv.MEMORY_ERASURE_BINDING='true';stageEnv.MEMORY_SOURCE_ERASURE='true';}
