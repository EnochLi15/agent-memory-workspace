"""Single B30 tail adapter over the frozen recovery executor. Planning is zero-cloud."""
...
spec=importlib.util.spec_from_file_location('b30_frozen_core',CORE);core=importlib.util.module_from_spec(spec);spec.loader.exec_module(core)
fixed,tail=core.fixed,core.tail
sha,read,write=core.sha,core.read_json,core.write_json
SAMPLE='B30_forget';CAMPAIGN='priority-B30-tail-qa-20260909-01';PREVIOUS=ROOT/'artifacts/priority-B30-segment30-add-20260909-01'
SCOPE='B30 original complete 51-add history and five fixed questions; 31 real committed prefix receipts and 20 remaining adds. No rewritten questions or previous-score changes.'
...
def partition(samples,schedule):
    ...
    if len(selected)!=1 or selected[0]['benchmark']!='memops' or len(selected[0]['sessions'])!=50 or len(selected[0]['questions'])!=5 or len({q['qid'] for q in selected[0]['questions']})!=5:raise ValueError('Exactly the original B30 50 sessions and five unique questions are required')
    own=[r for r in schedule if r['sample_id']==SAMPLE]
    if len(own)!=51 or not own[30]['request_id'].endswith(':segment-30:0') or not own[31]['request_id'].endswith(':segment-31:0'):raise ValueError('Expected 31 prefix receipts and 20 original tail adds')
    return selected,own
...
def successful_prefix(database,case,prefix,result,schedule,namespace):
    ...
    if check['integrity']!='pass' or check['revision_after']!=31:raise ValueError('Successful add must have its exact receipt and revision 31')
    return core.prefix_receipts(database,schedule,namespace,SAMPLE,31)


def source(previous):
    previous=previous_path(previous);idle(previous);p=read(previous/'plan.json')
    if p['protocol']!='fixed-failed-adds-http-v1' or p['planned_adds']!=1:raise ValueError('Expected the single B30 failed-add probe')
    data,origin,_,prefix=fixed.validate_inputs(p['inputs'],p['inputs_sha256']);case=data['cases'][0]
    if case['sample_id']!=SAMPLE or case['revision']!=30 or not case['request_id'].endswith(':segment-30:0'):raise ValueError('Single-add source identity changed')
    ...
    if hashes!=p['candidate_files'] or len(hashes)!=141:raise ValueError('Executed successful-add candidate changed')
    ...
    snapshot={'sample_id':SAMPLE,'revision':31,'source':str(database),'source_sha256':sha(database.read_bytes()),'logical_state':fixed.logical_state(database)}
...
def prepare(args):
    ...
    captured=campaign/'B30-rev31.sqlite'
    ...
    run_id=campaign.name+'-candidate-memops';phase={'id':'b30-tail','samples':[SAMPLE],'dataset':str(campaign/'samples.json'),'dataset_sha256':sha((campaign/'samples.json').read_bytes()),'planned_questions':5,'run_id':run_id,'run_dir':str(ROOT/'eval/artifacts'/run_id),'timeout_seconds':10800}
    pinned={str(campaign/f):sha((campaign/f).read_bytes()) for f in ['samples.json','schedule.json','receipts.private.json','candidate-manifest.json','B30-rev31.sqlite']}
    plan={'protocol':'single-B30-tail-qa-v1','campaign':campaign.name,'created_at':core.now(),'scope':SCOPE,'entry_point':str(probes/HERE.name),'previous':str(source_dir),'namespace':origin['namespace'],'eval_code_root':str(Path(origin['code_root'])/'eval'),'eval_commit':tail.EVAL_COMMIT,'spec':origin['spec'],'env_file':origin['env_file'],'candidate_commit':previous['candidate_provenance']['commit'],'candidate_dist':str(campaign/'service-dist'),'candidate_files':previous['candidate_files'],'origin_service_config':str(source_dir/'service.json'),'port':args.port,'data_dir':str(campaign/'data'),'phases':[phase],'planned_questions':5,'prefix_receipts':31,'total_adds':51,'new_tail_adds':20,'snapshots':[snapshot],'origin_files':files,'dependencies':dependencies,'pinned_files':pinned}
...
def validate(campaign,allow_owned_launch=False):
    ...
    if plan['protocol']!='single-B30-tail-qa-v1' or plan['campaign']!=campaign.name or (plan['planned_questions'],plan['prefix_receipts'],plan['total_adds'],plan['new_tail_adds'])!=(5,31,51,20):raise ValueError('Single-sample identity or denominator changed')
    phase={'id':'b30-tail','samples':[SAMPLE],'dataset':str(campaign/'samples.json'),'dataset_sha256':sha((campaign/'samples.json').read_bytes()),'planned_questions':5,'run_id':campaign.name+'-candidate-memops','run_dir':str(ROOT/'eval/artifacts'/(campaign.name+'-candidate-memops')),'timeout_seconds':10800}
    ...
    if core.prefix_receipts(frozen['path'],schedule,origin['namespace'],SAMPLE,31)!=receipts:raise ValueError('Captured prefix differs')
    return {'validation':'passed','model_calls_made':0,'candidate_commit':plan['candidate_commit'],'planned_questions':5,'prefix_receipts':31,'new_tail_adds':20,'total_adds':51,'previous_status':idle(source_dir),'entry_point':plan['entry_point']}


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--campaign',default=CAMPAIGN);parser.add_argument('--port',type=int,default=8121);parser.add_argument('--previous',default=str(PREVIOUS));parser.add_argument('--detach',action='store_true')
