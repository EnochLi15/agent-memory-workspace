...
const e=parseEnv(readFileSync('.env','utf8'));const config={...configFromEnv({...e,MEMORY_LLM_MODEL:'gpt-5.5',MEMORY_LLM_REASONING_EFFORT:'low',MEMORY_EMBEDDING_DIGEST:'0a109f422b47e3a30ba2b10eca18548e944e8a23073ee3f3e947efcf3c45e59f'}),port:0,dataDir:join(output,'data')};
...
const originalJson=Models.prototype.json;Models.prototype.json=async function(system,user,signal){const raw=await originalJson.call(this,system,user,signal);appendFileSync(join(output,'model-proposals.jsonl'),JSON.stringify({tag:'DEBUG-variant-probe',case:currentCase,input:user,output:raw})+'\n');return raw;};
const sourceFiles=readdirSync('service/src').filter(p=>p.endsWith('.ts')).map(p=>'service/src/'+p);const report={protocol:'real-model-lifecycle-variants-v1',model:config.llmModel,reasoning_effort:config.llmReasoningEffort,source_sha256:Object.fromEntries(sourceFiles.map(p=>[p,sha(readFileSync(p))])),probe_sha256:sha(readFileSync('scripts/probes/lifecycle-variants.mjs')),run_dir:output,cases:[],scope:'Five synthetic controls with fixed expected retention, not benchmark questions or overall accuracy.'};
try{
