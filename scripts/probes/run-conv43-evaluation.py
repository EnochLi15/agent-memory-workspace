"""One original conv-43 LoCoMo phase using the existing priority campaign runner."""
...
core=module('conv43_priority_core',PROBES/'run-priority-campaign.py');core.ROOT=ROOT
...
helper=module('conv43_frozen_helpers',HELPER);tail,fixed=helper.tail,helper.fixed
sha,read,write=core.sha,helper.read_json,core.write_json
CAMPAIGN='priority-conv43-20260909-01';COMMIT='8717d918160e36f4e8822d0ad80298342e474c5a'
CANDIDATE=ROOT/'artifacts/priority-focus-fallback-integration-20260909'
MANIFEST_SHA='0b43eb46dd1c7c189f386e57d4f61188edca24d1eda7f119e6be5fa7eb65c61c'
...
SCOPE='Complete original conv-43: 29 sessions, 680 messages, 43 fresh adds and 47 fixed questions. Original LoCoMo refined Python Judge with local qwen3:14b; GLM-5.2 Answer.'
...
def adapted_core(raw):
    ...
    changes=[(old,"safe = json.loads(Path(plan['service_config_base']).read_text())"),
             ...
             ('High-risk ingestion gate failed; remaining 935 questions were not launched','LoCoMo ingestion failed; all 47 planned questions retain their original denominator')]
...
def source():
    review=read(ROOT/'reports/priority-next-coverage-20260909.json');origin=read(ORIGINAL)
    if sha(ORIGINAL.read_bytes())!=review['original_plan']['sha256']:raise ValueError('Original campaign plan changed')
    locomo=next(p for p in origin['phases'] if p['id']=='locomo');raw=Path(locomo['data_file']).read_bytes()
    if sha(raw)!=locomo['dataset_sha256'] or sha(raw)!=review['recommended_next_sample']['source_dataset']['sha256']:raise ValueError('Original LoCoMo dataset changed')
    samples=[s for s in json.loads(raw) if s['sample_id']=='conv-43'];validate_sample(samples)
    if [q['qid'] for q in samples[0]['questions']]!=review['recommended_next_sample']['qids']:raise ValueError('Original conv-43 question order changed')
    ...
    for path in [Path(origin['spec']),Path(origin['selection_manifest']),Path(origin['env_file']),ROOT/'reports/priority-next-coverage-20260909.json']:
        pins[str(path)]=sha(path.read_bytes())
    return origin,samples,manifest,dist,data['dist'],pins


def validate_sample(samples):
    if len(samples)!=1:raise ValueError('Exactly one original conv-43 sample is required')
    s=samples[0]
    if s['sample_id']!='conv-43' or s['benchmark']!='locomo' or len(s['sessions'])!=29 or sum(len(x['messages']) for x in s['sessions'])!=680 or len(s['questions'])!=47 or len({q['qid'] for q in s['questions']})!=47:raise ValueError('Complete 29-session/680-message/47-question conv-43 required')
...
def phase(campaign):
    run_id=campaign.name+'-candidate-locomo';data=campaign/'conv43.json'
    return {'id':'locomo','benchmark':'locomo','data_file':str(data),'dataset_sha256':sha(data.read_bytes()),'questions':47,'samples':1,'run_id':run_id,'run_dir':str(ROOT/'eval/artifacts'/run_id)}
...
def prepare(args):
    ...
    campaign.mkdir(mode=0o700);write(campaign/'conv43.json',samples);namespace=campaign.name+'-candidate';code=campaign/'code';(code/'service').mkdir(parents=True);(code/'configs').mkdir()
    ...
    schedule=tail.request_schedule(code/'eval',campaign/'conv43.json',namespace)
    if len(schedule)!=43 or len({r['request_id'] for r in schedule})!=43:raise ValueError('Frozen evaluator must generate exactly 43 requests')
    ...
    dependencies.update({str(campaign/name):sha((campaign/name).read_bytes()) for name in ['conv43.json','schedule.json','receipts.private.json','service-base.json','observe-search.mjs','service-launcher.mjs','candidate-manifest.json']})
    plan={'protocol':'single-conv43-locomo-v1','campaign':campaign.name,'created_at':core.now(),'scope':SCOPE,'entry_point':str(probes/HERE.name),'code_root':str(code),'candidate_commit':COMMIT,'candidate_manifest_sha256':MANIFEST_SHA,'candidate_files':hashes,'namespace':namespace,'spec':str(code/'configs/spec.json'),'spec_sha256':frozen['configs/spec.json'],'env_file':origin['env_file'],'service_config_base':str(campaign/'service-base.json'),'configuration_override':OVERRIDE,'selection_manifest':origin['selection_manifest'],'selection_manifest_sha256':origin['selection_manifest_sha256'],'source_checkpoints':{'workspace':origin['source_checkpoints']['workspace'],'eval':origin['source_checkpoints']['eval'],'service':{'commit':COMMIT}},'frozen_code_hashes':frozen,'source_files':pins,'dependencies':dependencies,'port':args.port,'data_dir':str(campaign/'data'),'phases':[phase(campaign)],'planned_questions':47,'planned_adds':43,'timeout_seconds':10800,'core_source_sha256':CORE_SHA,'adapted_core_sha256':sha(patched.encode()),'judge_protocol':{'mode':'upstream-reproduction','judge_kind':'refined-python','judge_model':'qwen3:14b','answer_model':'glm-5.2'}}
    write(campaign/'plan.json',plan);spec,datasets=validate(campaign);result={'validation':'passed','model_calls_made':0,'planned_questions':47,'fresh_adds':43,'candidate_commit':COMMIT,'entry_point':plan['entry_point'],'judge_protocol':plan['judge_protocol']};write(campaign/'validation.json',result);return result
...
def validate(campaign,fresh=True):
    ...
    expected={'protocol':'single-conv43-locomo-v1','campaign':campaign.name,'entry_point':str(campaign/'executor/scripts/probes'/HERE.name),'code_root':str(campaign/'code'),'candidate_commit':COMMIT,'candidate_manifest_sha256':MANIFEST_SHA,'candidate_files':hashes,'namespace':campaign.name+'-candidate','spec':str(campaign/'code/configs/spec.json'),'service_config_base':str(campaign/'service-base.json'),'configuration_override':OVERRIDE,'env_file':origin['env_file'],'port':8125,'data_dir':str(campaign/'data'),'phases':[phase(campaign)],'planned_questions':47,'planned_adds':43,'timeout_seconds':10800,'source_files':pins,'judge_protocol':{'mode':'upstream-reproduction','judge_kind':'refined-python','judge_model':'qwen3:14b','answer_model':'glm-5.2'}}
    if any(plan.get(k)!=v for k,v in expected.items()):raise ValueError('Frozen conv-43 plan differs')
    ...
    if fixed.all_hashes(code/'service/dist')!=hashes or read(plan['spec'])!=effective_spec(origin) or read(campaign/'conv43.json')!=samples:raise ValueError('Candidate, three configuration overrides or original sample changed')
    ...
    schedule=tail.request_schedule(code/'eval',campaign/'conv43.json',plan['namespace'])
    if len(schedule)!=43 or schedule!=read(campaign/'schedule.json') or core.expected_requests(samples,plan['namespace'])!={r['request_id'] for r in schedule}:raise ValueError('Exact original schedule changed')
    ...
    if fresh:
        ...
        if Path(plan['data_dir']).exists():raise FileExistsError('Fresh conv-43 cannot reuse storage')
    return read(plan['spec']),[samples]


BASE_SUMMARIZE=core.summarize
def summarize(plan,campaign,statuses):
    result=BASE_SUMMARIZE(plan,campaign,statuses);result.update(planned_questions=47,accuracy_over_planned=result['correct']/47,scope=SCOPE)
    judged={r['qid']:r for r in core.read_rows(Path(plan['phases'][0]['run_dir'])/'judgments.jsonl',live=True)}
    result['question_statuses']=[{'qid':q['qid'],'status':judged.get(q['qid'],{}).get('status','not_completed'),'correct':judged.get(q['qid'],{}).get('correct')} for s in read(campaign/'conv43.json') for q in s['questions']]
    return result
...
def storage_check(plan,campaign):
    ...
    user=plan['namespace']+':locomo:conv-43';db=Path(plan['data_dir'])/sha(user.encode())/'memory.sqlite';saved={};revision=0
    if db.exists():
    ...
    queries={'search:'+sha(q['question'].encode()):q['question'] for s in read(campaign/'conv43.json') for q in s['questions']}
...
def main():
    ...
    if args.validate:print(json.dumps({'validation':'passed','model_calls_made':0,'planned_questions':47,'fresh_adds':43}));return
    ...
    class TimedRun(core.ManagedRun):
        ...
        def poll(self,check_required=True):
            if check_required and time.monotonic()>self.deadline:raise TimeoutError('Complete conv-43 run exceeded its pinned deadline')
