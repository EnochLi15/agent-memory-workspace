...
if(runtimeIndex>=0&&!process.argv[runtimeIndex+1])throw Error('Missing --runtime path');
const runtime=resolve(runtimeIndex>=0?process.argv[runtimeIndex+1]:'service');
...
for(const [path,hash] of Object.entries(manifest.source_sha256)){if(!path.startsWith('service/src/')||path.includes('..'))throw Error('Unexpected service source path');const actual=sha(readFileSync(join(runtime,path.slice('service/'.length))));if(actual!==hash){if(path!=='service/src/prompts.ts')throw Error('Service source changed since ingestion: '+path);sourceDifferences.push({path,ingest_sha256:hash,query_sha256:actual,reason:'Only the extraction prompt changed; this read-only probe never invokes extraction.'});}}
const output=join(dir,'retrieval-probe-v1');mkdirSync(output);
const e=parseEnv(readFileSync('.env','utf8'));
const config={...configFromEnv({...e,MEMORY_EMBEDDING_DIGEST:'0a109f422b47e3a30ba2b10eca18548e944e8a23073ee3f3e947efcf3c45e59f'}),llmModel:manifest.default_model??manifest.extraction_model,...(manifest.reasoning_effort&&manifest.reasoning_effort!=='provider_default'?{llmReasoningEffort:manifest.reasoning_effort}:{}),dataDir:join(dir,'data'),port:0};
...
const report={protocol:'exposed-failed-prefix-retrieval-v1',parent_results_sha256:sha(readFileSync(join(dir,'results.json'))),sample,source_differences:sourceDifferences,answer_model:'gpt-5.4-mini',answer_prompt_sha256:sha(ANSWER_PROMPT),retrieval_model:config.llmModel,retrieval_reasoning_effort:config.llmReasoningEffort??'provider_default',questions:[],scope:'Posthoc diagnostic queries, not benchmark scores. Inspect stored answers before assigning semantic pass.'};
report.query_runtime=runtime;
try{
