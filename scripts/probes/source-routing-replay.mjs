// Exact failed requests against copied v8 snapshots. Prepare-only: never relabel
// the source format or commit a v9 plan into an existing v8 tenant.
...
const report={protocol:'source-routing-replay-v2',selected_cases:selected,spec_path:specPath,extraction_format:config.extractionFormat,llm_connection:connection,run_dir:dir,scope:'Real model prepare-only replay of selected exposed failed requests against exact copied v8 snapshots. No migration, commit, full HTTP ingestion, benchmark accuracy or independent full repeat.',source_sha256:Object.fromEntries(files.map(f=>[f,sha(readFileSync('service/src/'+f))])),spec_sha256:sha(readFileSync(specPath)),started_at:new Date().toISOString(),status:'running',cases:[]};
...
for(const parent of ['artifacts/round2-E10-v8-failure-8nr_ipub','artifacts/round2-B05-v8-failure-6arzpegx'].filter(p=>selected==='all'||p.includes('round2-'+selected+'-'))){
