...
name='comp-memory-offline-audit';volume='comp-memory-offline-audit-data';image='comp-agent-memory-service:dev'
...
run('docker','stop',name);run('docker','rm',name)
result={'image':run('docker','image','inspect',image,'--format','{{.Id}}'),'network':'none','checks':['cold-start','synchronous-add','forget','sigkill-restart','retained-neighbor','tenant-isolation','remove-and-recreate-with-volume'],'status':'passed','retained_volume':volume}
