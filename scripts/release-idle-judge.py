"""Release this task's local Judge only after its evaluation finishes."""
import datetime,hashlib,json,pathlib,subprocess,time,urllib.request
root=pathlib.Path(__file__).resolve().parents[1];manifest=root/'eval/artifacts/holdout-v2-U3-locomo/manifest.json';log=root/'artifacts/holdout-v2/judge-residency.jsonl'
def record(data):
 with log.open('a') as f:f.write(json.dumps({'at':datetime.datetime.now(datetime.timezone.utc).isoformat(),**data})+'\n')
record({'event':'policy_started','trigger':'after_locomo_completion','model':'qwen3:14b','script_sha256':hashlib.sha256(pathlib.Path(__file__).read_bytes()).hexdigest(),'reason':'New Metal initialization stalls in the independent import diagnostic; avoid elective unload/reload during the frozen evaluation.','scope':'Only residency changes; same model digest, prompts and inference parameters.'})
while True:
