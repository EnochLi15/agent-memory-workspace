...
当前唯一交付主线是 [V1交付收敛计划](docs/29-V1交付收敛计划.md)：修复交付阻塞、固定发布配置、通过小集闭环与原文对照、完成一次1000题评测，随后封版。当前候选已通过干净部署、备份恢复及固定小集闭环，长背景阻塞、完整评测和正式封版仍待验收；修复证据见 [V1阻塞进展](reports/v1-blocker-followup.md)，固定对照见 [当前小集结果](reports/v1-small-b398a63-results.md) 和 [旧候选长背景结果](reports/v1-long-7a50d22-results.md)。报告均绑定对应代码版本，后续修复不沿用旧版本成绩。
