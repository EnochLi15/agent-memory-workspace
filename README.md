...
后续协议兼容修复见[固定小集闭环记录](reports/v1-compat-fix-02.md)：430项回归通过，5次写入与10题回答判分全部执行完成，20项遗忘/更新/邻居保留检查通过。原始评分5/10，其中4条判错存在证据争议，另1题有真实内容遗漏；尚未达到质量验收，未追加100题评测。

具名核验协议已实现并完成[固定小集同预算对照](reports/v1-named-protocol-pair-01.md)：服务439项回归通过，新旧协议均完成5次写入、10题判分及20项存储检查。原始评分新6/10、旧5/10；两组均无结构错误，尚不能证明稳定收益，默认仍保留 compact，named 显式可选。

有限次数的核验恢复已落地：[修改与验证记录](reports/v1-verification-recovery-01.md)。519 项回归通过，固定小集完成 5/5 写入、10/10 判分和 20/20 存储检查，原始评分 5/10。原始损坏 JSON 的真实补验仍因新增未知字段被拒绝；没有放宽删除校验，也未扩大评测。
