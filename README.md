...
限流修正：BigModel候选现采用服务内共享串行队列、请求启动间隔至少1秒、429共享等待；评测固定单背景并发，LoCoMo结束后再启动MemOps。说明及本地验证见[限流修正记录](reports/v1-glm52-rate-limit-fix.md)。此修改尚未经过新的真实模型评测，分组/核验覆盖问题仍未解决。
